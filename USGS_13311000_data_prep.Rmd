...
# Step 2: Read PRISM data
```{r}
prism<-read.csv("PRISM_ppt_tmin_tmean_tmax_tdmean_44.9057_-115.3295.csv",sep=",")
prism$Date <-as.Date(prism$Date, format="%Y-%m-%d")
summary(prism)
```

```{r}
st_all_gage<-c("13311000")
#create empty tables
...
# Step 3: Load USGS data
```{r}
#saveRDS( df, file="USGS_13311000_ST_Q.rds")
df<-readRDS("USGS_13311000_ST_Q.rds")
summary(df)
```
...
# Step 4: Load PRISM and USGS
```{r}
saveRDS( prism_df, file="prism_df_13311000.rds")
...
rinf$site_id<- 13311000
rinf
saveRDS(rinf ,file=  "rinf_13311000.rds"  )
...
daily_df_summer_2017<-daily_df_summer[daily_df_summer$yr==2022,]
